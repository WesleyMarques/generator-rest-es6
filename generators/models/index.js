    ...
    module.exports = class extends Generator {
      ...
      writing() {
        ...
        copyTemplate(this.fs, this.templatePath('_migration.js'), this.destinationPath('../migrations/' + Date.now() + '-create-' + pluralize.plural(dasherize(this.options.modelObj.name)) + '.js'), {
          ...
          var importValue = "import " + _.camelCase(this.options.modelObj.name) + " from './server/routes/" + this.options.modelObj.name + "';";
          var useValue = _.camelCase(this.options.modelObj.name) + "(app);";
          var file = this.fs.read(this.destinationPath('../app.js')); file = file.replace('//import-inject', importValue + '\n//import-inject'); file = file.replace('//router-inject', useValue + '\n//router-inject'); this.fs.write(this.destinationPath('../app.js'), file);
        }
